...
interface BiomeData {
  biomeTypes: string[];  // e.g. ["farm", "river", "campsite", "ruins", ...]
  assignments: Record<string, string[]>; // "kind:setName" → biome names
}

/** Season assignments: set-level */
interface SeasonData {
  seasonTypes: string[];  // e.g. ["spring", "summer", "autumn", "winter"]
  assignments: Record<string, string[]>; // "kind:setName" → season names
}
...
export const AtlasApprovedDatabase: React.FC = () => {
  ...
  const [biomeData, setBiomeData] = useState<BiomeData>({ biomeTypes: [], assignments: {} });
  const [editingBiomeSet, setEditingBiomeSet] = useState<string | null>(null);
  const [newBiomeName, setNewBiomeName] = useState('');
  const [showBiomeManager, setShowBiomeManager] = useState(false);
  const [seasonFilter, setSeasonFilter] = useState<string>('all');
  const [seasonData, setSeasonData] = useState<SeasonData>({ seasonTypes: ['spring', 'summer', 'autumn', 'winter'], assignments: {} });
  const [editingSeasonSet, setEditingSeasonSet] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    axios.get('/api/atlas/biomes').then((res) => {
      if (res.data && typeof res.data === 'object') {
        setBiomeData(res.data as BiomeData);
      }
    }).catch(() => {});
    // Load season assignments
    axios.get('/api/atlas/seasons').then((res) => {
      if (res.data && typeof res.data === 'object') {
        setSeasonData(res.data as SeasonData);
      }
    }).catch(() => {});
  }, []);

  const saveBiomeData = async (data: BiomeData) => {
    setBiomeData(data);
    try { await axios.post('/api/atlas/biomes', data); } catch { /* silent */ }
  };

  const setKey = (entry: ApprovedSet) => `${entry.kind}:${entry.name}`;

  const toggleBiome = (entry: ApprovedSet, biome: string) => {
    const key = setKey(entry);
    const current = biomeData.assignments[key] || [];
    const next = current.includes(biome)
      ? current.filter(b => b !== biome)
      : [...current, biome];
    const updated: BiomeData = {
      ...biomeData,
      assignments: { ...biomeData.assignments, [key]: next },
    };
    // Clean up empty assignments
    if (next.length === 0) delete updated.assignments[key];
    saveBiomeData(updated);
  };

  const addBiomeType = (name: string) => {
    const trimmed = name.trim().toLowerCase();
    if (!trimmed || biomeData.biomeTypes.includes(trimmed)) return;
    saveBiomeData({ ...biomeData, biomeTypes: [...biomeData.biomeTypes, trimmed].sort() });
    setNewBiomeName('');
  };

  const removeBiomeType = (name: string) => {
    if (!confirm(`Remove biome type "${name}"? This will also remove it from all set assignments.`)) return;
    const types = biomeData.biomeTypes.filter(b => b !== name);
    const assignments = { ...biomeData.assignments };
    for (const key of Object.keys(assignments)) {
      assignments[key] = assignments[key].filter(b => b !== name);
      if (assignments[key].length === 0) delete assignments[key];
    }
    saveBiomeData({ biomeTypes: types, assignments });
  };

  // ── Season CRUD ──

  const saveSeasonData = async (data: SeasonData) => {
    setSeasonData(data);
    try { await axios.post('/api/atlas/seasons', data); } catch { /* silent */ }
  };

  const toggleSeason = (entry: ApprovedSet, season: string) => {
    const key = setKey(entry);
    const current = seasonData.assignments[key] || [];
    const next = current.includes(season)
      ? current.filter(s => s !== season)
      : [...current, season];
    const updated: SeasonData = {
      ...seasonData,
      assignments: { ...seasonData.assignments, [key]: next },
    };
    if (next.length === 0) delete updated.assignments[key];
    saveSeasonData(updated);
  };

  const addSeasonType = (name: string) => {
    const trimmed = name.trim().toLowerCase();
    if (!trimmed || seasonData.seasonTypes.includes(trimmed)) return;
    saveSeasonData({ ...seasonData, seasonTypes: [...seasonData.seasonTypes, trimmed].sort() });
    setNewSeasonName('');
  };

  const removeSeasonType = (name: string) => {
    if (!confirm(`Remove season type "${name}"? This will also remove it from all set assignments.`)) return;
    const types = seasonData.seasonTypes.filter(s => s !== name);
    const assignments = { ...seasonData.assignments };
    for (const key of Object.keys(assignments)) {
      assignments[key] = assignments[key].filter(s => s !== name);
      if (assignments[key].length === 0) delete assignments[key];
    }
    saveSeasonData({ seasonTypes: types, assignments });
  };
  ...
  const filtered = useMemo(() => {
    return approvedSets.filter((s) => {
      ...
      if (biomeFilter !== 'all') {
        ...
        const assigned = biomeData.assignments[key] || [];
        if (biomeFilter === '__unassigned__') {
          if (assigned.length > 0) return false;
        } else {
          if (!assigned.includes(biomeFilter)) return false;
        }
      }
      if (seasonFilter !== 'all') {
        const key = `${s.kind}:${s.name}`;
        const assigned = seasonData.assignments[key] || [];
        if (seasonFilter === '__unassigned__') {
          // Show only sets with NO explicit season tags
          if (assigned.length > 0) return false;
        } else if (seasonFilter === '__explicit__') {
          // Show only sets that HAVE at least one explicit season tag
          if (assigned.length === 0) return false;
        } else {
          // Unassigned = all seasons, so include them; otherwise must match
          if (assigned.length > 0 && !assigned.includes(seasonFilter)) return false;
        }
      }
  ...
  }, [approvedSets, kindFilter, atlasFilter, groupFilter, search, biomeFilter, biomeData, seasonFilter, seasonData]);
  ...
  const buildExportPayload = (sets: ApprovedSet[]) => {
    const setsWithBiomes = sets.map(s => {
      const biomes = biomeData.assignments[`${s.kind}:${s.name}`] || [];
      const seasons = seasonData.assignments[`${s.kind}:${s.name}`] || [];
      return { ...s, ...(biomes.length > 0 ? { biomes } : {}), ...(seasons.length > 0 ? { seasons } : {}) };
    });
    return {
      generatedAt: new Date().toISOString(),
      totalSets: sets.length,
      biomeTypes: biomeData.biomeTypes,
      seasonTypes: seasonData.seasonTypes,
      sets: setsWithBiomes,
    };
  };

  const exportApprovedSets = () => {
    const data = buildExportPayload(filtered);
  ...
  const publishApprovedSets = async () => {
    setPublishStatus('');
    setIsPublishing(true);
    try {
      const payload = buildExportPayload(approvedSets);
      await axios.post('/api/atlas/approved-sets', payload);
      setPublishStatus(`✅ Published ${payload.totalSets} sets to map-generator pipeline`);
    } catch {
      setPublishStatus('❌ Failed to publish approved-atlas-sets.json');
    } finally {
    ...
    <div style={{ height: '100%', background: '#0f0f1a', color: '#e0e0e0', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #1d2a4d', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        >
          ...
          <option value="__unassigned__">⚠ No season tags (all-season)</option>
          <option value="__explicit__">✓ Has season tags</option>
          {seasonData.seasonTypes.map(s => (
            <option key={s} value={s}>{s} (incl. untagged)</option>
          ))}
        </select>
        <button onClick={() => setShowBiomeManager(!showBiomeManager)} style={{ background: '#1a3a2a', color: '#8fd8a8', border: '1px solid #2a694f', borderRadius: 4, padding: '6px 10px', cursor: 'pointer', fontSize: '0.82rem' }}>
          🏷 Biomes
        </button>
        <button onClick={() => setShowSeasonManager(!showSeasonManager)} style={{ background: '#1a2a3a', color: '#8fb8d8', border: '1px solid #2a4f69', borderRadius: 4, padding: '6px 10px', cursor: 'pointer', fontSize: '0.82rem' }}>
          🍃 Seasons
        </button>
        <button
          onClick={publishApprovedSets}
          disabled={isPublishing}
          style={{
            background: '#23401f',
            color: '#b5f59c',
            border: '1px solid #3b7a33',
            borderRadius: 4,
            padding: '6px 10px',
          ...
          title="Write approved-atlas-sets.json directly to frontend/public for map generator"
        >
          {isPublishing ? 'Publishing…' : '🚀 Publish to Map Generator'}
        </button>
        ...
        {publishStatus && <span style={{ fontSize: '0.78rem', color: '#9ad88f' }}>{publishStatus}</span>}
      </div>
      ...
      {/* Biome Manager Panel */}
      {showBiomeManager && (
        <div style={{ padding: '10px 16px', borderBottom: '1px solid #1d2a4d', background: '#0f1f15' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 'bold', marginBottom: 6, color: '#8fd8a8' }}>Biome / Feature Types</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
            {biomeData.biomeTypes.map(b => {
              const count = Object.values(biomeData.assignments).filter(arr => arr.includes(b)).length;
              return (
                <span
                  key={b}
                  style={{
                    ...
                    background: '#1a3a2a', border: '1px solid #2a694f', color: '#8fd8a8',
                  }}
                >
                  {b} <span style={{ opacity: 0.6 }}>({count})</span>
                  <button
                    onClick={() => removeBiomeType(b)}
                    style={{ background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', padding: 0, fontSize: '0.8rem', lineHeight: 1 }}
                    title={`Remove "${b}" biome type`}
                  >
                    ×
                  </button>
                </span>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') addBiomeType(newBiomeName); }}
              placeholder="New biome name..."
              style={{ background: '#15152a', color: '#eee', border: '1px solid #2a694f', borderRadius: 4, padding: '4px 8px', fontSize: '0.82rem', width: 180 }}
            />
            <button
              onClick={() => addBiomeType(newBiomeName)}
              style={{ background: '#1a3a2a', color: '#8fd8a8', border: '1px solid #2a694f', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: '0.82rem' }}
            >
              + Add
            </button>
          </div>
      ...
      {showSeasonManager && (
        <div style={{ padding: '10px 16px', borderBottom: '1px solid #1d2a4d', background: '#0f1520' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 'bold', marginBottom: 6, color: '#8fb8d8' }}>Season Types</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
            {seasonData.seasonTypes.map(s => {
              const count = Object.values(seasonData.assignments).filter(arr => arr.includes(s)).length;
              return (
                <span
                  key={s}
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: 4,
                    fontSize: '0.72rem', padding: '2px 8px', borderRadius: 999,
                    background: '#1a2a3a', border: '1px solid #2a4f69', color: '#8fb8d8',
                  }}
                >
                  {s} <span style={{ opacity: 0.6 }}>({count})</span>
                  <button
                    onClick={() => removeSeasonType(s)}
                    style={{ background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', padding: 0, fontSize: '0.8rem', lineHeight: 1 }}
                    title={`Remove "${s}" season type`}
                  >
                    ×
                  </button>
                </span>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') addSeasonType(newSeasonName); }}
              placeholder="New season name..."
              style={{ background: '#15152a', color: '#eee', border: '1px solid #2a4f69', borderRadius: 4, padding: '4px 8px', fontSize: '0.82rem', width: 180 }}
            />
            <button
              onClick={() => addSeasonType(newSeasonName)}
              style={{ background: '#1a2a3a', color: '#8fb8d8', border: '1px solid #2a4f69', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: '0.82rem' }}
            >
              + Add
            </button>
          </div>
      ...
      <div style={{ flex: 1, minHeight: 0, padding: 12, overflow: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10, alignContent: 'start' }}>
        {filtered.map((entry) => (
          <div
            ...
            style={{ border: '1px solid #2a3f69', background: '#15152a', borderRadius: 6, padding: 10 }}
          >
            ...
            <div style={{ marginTop: 6, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setEditingBiomeSet(editingBiomeSet === setKey(entry) ? null : setKey(entry));
                }}
                style={{ background: '#1a3a2a', color: '#8fd8a8', border: '1px solid #2a694f', borderRadius: 4, padding: '2px 6px', fontSize: '0.68rem', cursor: 'pointer' }}
              >
                🏷 Biomes
              </button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setEditingSeasonSet(editingSeasonSet === setKey(entry) ? null : setKey(entry));
                }}
                style={{ background: '#1a2a3a', color: '#8fb8d8', border: '1px solid #2a4f69', borderRadius: 4, padding: '2px 6px', fontSize: '0.68rem', cursor: 'pointer' }}
              >
                🍃 Seasons
              </button>
              {/* Biome badges */}
              {(biomeData.assignments[setKey(entry)] || []).map(b => (
                <span
                  key={b}
                  style={{
                    fontSize: '0.6rem', padding: '1px 5px', borderRadius: 999,
                    background: '#1a3a2a', border: '1px solid #2a694f', color: '#8fd8a8',
                  }}
                >
                  {b}
              ...
              {/* Season badges */}
              {(seasonData.assignments[setKey(entry)] || []).map(s => (
                <span
                  key={s}
                  style={{
                    fontSize: '0.6rem', padding: '1px 5px', borderRadius: 999,
            ...
            {/* Biome toggle panel for this set */}
            {editingBiomeSet === setKey(entry) && biomeData.biomeTypes.length > 0 && (
              <div
                onClick={(e) => e.stopPropagation()}
                style={{
                  marginTop: 6, padding: 6, background: '#0f1f15', border: '1px solid #2a694f',
                  borderRadius: 4, display: 'flex', flexWrap: 'wrap', gap: 4,
                }}
              >
                {biomeData.biomeTypes.map(b => {
                  const active = (biomeData.assignments[setKey(entry)] || []).includes(b);
                  return (
                    <button
                      key={b}
                      onClick={(e) => { e.stopPropagation(); toggleBiome(entry, b); }}
                      style={{
                        fontSize: '0.68rem', padding: '2px 8px', borderRadius: 999, cursor: 'pointer',
                        background: active ? '#2a694f' : '#15152a',
                        color: active ? '#fff' : '#8fd8a8',
                        border: `1px solid ${active ? '#3aaa6f' : '#2a694f'}`,
            ...
            {/* Season toggle panel for this set */}
            {editingSeasonSet === setKey(entry) && seasonData.seasonTypes.length > 0 && (
              <div
                onClick={(e) => e.stopPropagation()}
                style={{
                  marginTop: 6, padding: 6, background: '#0f1520', border: '1px solid #2a4f69',
                  borderRadius: 4, display: 'flex', flexWrap: 'wrap', gap: 4,
                }}
              >
                {seasonData.seasonTypes.map(s => {
                  const active = (seasonData.assignments[setKey(entry)] || []).includes(s);
                  return (
                    <button
                      key={s}
                      onClick={(e) => { e.stopPropagation(); toggleSeason(entry, s); }}
                      style={{
                        fontSize: '0.68rem', padding: '2px 8px', borderRadius: 999, cursor: 'pointer',
                        background: active ? '#2a4f69' : '#15152a',
                        color: active ? '#fff' : '#8fb8d8',
                        border: `1px solid ${active ? '#3a8aaa' : '#2a4f69'}`,
                        fontWeight: active ? 'bold' : 'normal',
                      }}
                    >
                      {active ? '✓ ' : ''}{s}
