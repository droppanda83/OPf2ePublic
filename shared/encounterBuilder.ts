...
export function getCreatureXP(creatureLevel: number, partyLevel: number): number {
  const diff = creatureLevel - partyLevel;
  if (diff < -4) return 10; // floor at 10
  if (diff > 4) return 160; // cap at 160
  return CREATURE_XP[diff] ?? 40;
}

/** Calculate XP budget for a given difficulty and party size */
export function getXPBudget(difficulty: Difficulty, partySize: number = 4): number {
...
export function buildEncounter(
  ...
  let candidates = getCreaturesInRange(minLv, maxLv);
  ...
  if (candidates.length === 0) {
    candidates = getCreaturesInRange(partyLevel - 4, partyLevel + 4);
  }
...
export const DIFFICULTIES: Difficulty[] = ['trivial', 'low', 'moderate', 'severe', 'extreme'];
