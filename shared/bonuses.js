...
exports.getProficiencyBonus = getProficiencyBonus;
exports.resolveStacking = resolveStacking;
exports.calculateMAP = calculateMAP;
exports.createDefaultAbilities = createDefaultAbilities;
exports.createDefaultProficiencies = createDefaultProficiencies;
// ─── Core Functions ──────────────────────────────────
/**
 ...
 * Legendary = 8 + level
 */
function getProficiencyBonus(rank, level) {
    const rankBonus = {
        untrained: 0,
        trained: 2,
        expert: 4,
 ...
 */
function resolveStacking(bonuses, penalties) {
    let total = 0;
    ...
    for (const p of penalties) {
        ...
        if (p.type === 'untyped') {
            ...
            if (p.value > current)
                penaltyByType.set(p.type, p.value);
        }
    }
    // Apply the worst of each typed penalty
    for (const v of penaltyByType.values())
        total -= v;
    return total;
}
/**
 ...
 */
function calculateMAP(attacksMadeThisTurn, isAgile = false) {
    if (attacksMadeThisTurn <= 0)
        return 0;
    if (attacksMadeThisTurn === 1)
        return isAgile ? -4 : -5;
    return isAgile ? -8 : -10;
}
// ─── Defaults ────────────────────────────────────────
function createDefaultAbilities(overrides) {
    return {
        strength: 2,
        dexterity: 2,
        constitution: 2,
...
function createDefaultProficiencies(overrides) {
    return {
        unarmed: 'trained',
