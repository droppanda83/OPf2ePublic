...
export type AtlasMapTheme = 'wilderness' | 'dungeon' | 'indoor' | 'cave' | 'urban';
...
export function generateAtlasMap(opts: MapGenOptions, atlas: AtlasData): GeneratedMap {
  ...
  const genTimeMs = Math.round(Date.now() - t0);
...
// ─── Atlas→Game Converter ───────────────────────────────────
//
// Converts the atlas generator's output into the formats consumed
// by the game engine:
//   TileType[][]        → for LoS, mechanics, fallback rendering
//   AtlasOverlay[]      → atlas sprite rendering
//   moveCostOverride    → movement cost per cell
//   TerrainTile[][]     → legacy passability grid
//   startingZones       → player/enemy spawn positions

import type { TileType, AtlasOverlay } from './mapGenerator';
import type {
  TerrainTile,
  MapFeature,
...
/**
 * Map a terrain name (from atlas data) to the closest TileType.
 * Falls back to 'floor' for unknown terrain.
 */
function terrainNameToTileType(name: string | null): TileType {
  if (!name) return 'wall';
  const lower = name.toLowerCase();

  // Water
  if (lower.includes('water')) return 'water-shallow';
  // Grass variants
  if (lower.includes('tall grass') || lower.includes('tall wheat')) return 'grass-tall';
  if (lower.includes('grass')) return 'grass';
  // Dirt / earth
  if (lower.includes('fertile') || lower.includes('light dirt')) return 'dirt';
  ...
  // Stone
  if (lower.includes('stone') || lower.includes('cobble')) return 'stone';
  // Planks/wood
  if (lower.includes('plank') || lower.includes('wood')) return 'planks';
  // Ice
  if (lower.includes('ice') || lower.includes('frozen')) return 'ice';
  // Mud
  if (lower.includes('mud') || lower.includes('swamp')) return 'mud';
  // Carpet (indoor)
...
/**
 * Convert a MechanicsCell to a TileType, using the terrain name for context.
 * Mechanics take priority over terrain name for blocking/hazard tiles.
 */
function mechanicsToTileType(mech: MechanicsCell, terrainName: string | null): TileType {
  if (mech.movement === 'blocked') {
    if (mech.opaque) return 'wall';
    if (mech.cover === 'standard' || mech.cover === 'greater') return 'rock';
    if (mech.cover === 'lesser') return 'barrel';
    return 'pillar';
  }
  if (mech.movement === 'door') return 'door';
  if (mech.movement === 'swim') return 'water-shallow';
  if (mech.hazardType === 'fire') return 'lava';
  if (mech.hazardType === 'water') return 'water-shallow';

  // If movement is difficult, check if the terrain implies the type
  if (mech.movement === 'difficult') {
    const baseTile = terrainNameToTileType(terrainName);
    // Some tile types are already difficult terrain
    if (baseTile === 'grass-tall' || baseTile === 'mud' || baseTile === 'ice' || baseTile === 'sand') return baseTile;
    return 'rubble'; // Generic difficult terrain fallback
  }

  // Normal movement — use terrain name to determine tile type
  return terrainNameToTileType(terrainName);
}

/**
 * Convert MechanicsCell movement type to a numeric movement cost.
 * Returns null to use the TileType default.
 */
function mechanicsToMoveCost(mech: MechanicsCell): number | null {
  switch (mech.movement) {
    ...
    case 'door': return 1;
    case 'normal': return null; // use tile default
    case 'fly-only': return Infinity;
    default: return null;
  }
}

/**
 * Find spawn positions for players along the left/bottom edges,
 * and enemy positions along the right/top edges.
 */
function generateStartingZones(
  mechanics: MechanicsCell[][],
  width: number,
  height: number,
  playerCount: number,
  enemyCount: number,
): { players: Position[]; enemies: Position[] } {
  const isWalkable = (x: number, y: number): boolean => {
    if (x < 0 || x >= width || y < 0 || y >= height) return false;
    const m = mechanics[y][x];
    return m.movement !== 'blocked' && m.movement !== 'fly-only' && m.movement !== 'swim';
  };
  ...
  // Players: scan from left columns inward
  for (let x = 0; x < Math.min(5, width) && players.length < Math.max(playerCount, 6); x++) {
    for (let y = Math.floor(height * 0.2); y < Math.floor(height * 0.8) && players.length < Math.max(playerCount, 6); y++) {
      if (isWalkable(x, y) && !used.has(`${x},${y}`)) {
        players.push({ x, y });
        used.add(`${x},${y}`);
      }
    }
  ...
  // Enemies: scan from right columns inward
  for (let x = width - 1; x >= Math.max(width - 5, 0) && enemies.length < Math.max(enemyCount, 8); x--) {
    for (let y = Math.floor(height * 0.2); y < Math.floor(height * 0.8) && enemies.length < Math.max(enemyCount, 8); y++) {
      if (isWalkable(x, y) && !used.has(`${x},${y}`)) {
        enemies.push({ x, y });
        used.add(`${x},${y}`);
      }
...
/** Result of converting a GeneratedMap into game-engine-compatible structures */
export interface AtlasMapGameData {
  template: EncounterMapTemplate;
  tiles: TileType[][];
...
/**
 * Convert a GeneratedMap from the atlas generator into the data structures
 * the game engine needs.
 *
 * @param generated - Output from generateAtlasMap()
 * @param playerCount - Number of players (for starting zone sizing)
 * @param enemyCount - Number of enemies (for starting zone sizing)
 */
export function atlasMapToGameData(
  generated: GeneratedMap,
  playerCount: number = 4,
  enemyCount: number = 4,
): AtlasMapGameData {
  const { width, height, terrainGrid, mechanics, overlays: atlasOverlays } = generated;

  // ── Build TileType[][] from mechanics + terrain ──
  const tiles: TileType[][] = [];
  for (let y = 0; y < height; y++) {
    tiles[y] = [];
    for (let x = 0; x < width; x++) {
      tiles[y][x] = mechanicsToTileType(mechanics[y][x], terrainGrid[y]?.[x] ?? null);
    }
  }

  // ── Build moveCostOverride from mechanics ──
  const moveCostOverride: (number | null)[][] = [];
  for (let y = 0; y < height; y++) {
    moveCostOverride[y] = [];
    for (let x = 0; x < width; x++) {
      moveCostOverride[y][x] = mechanicsToMoveCost(mechanics[y][x]);
    }
  }

  // ── Build legacy TerrainTile[][] ──
  const terrain: TerrainTile[][] = [];
  for (let y = 0; y < height; y++) {
    terrain[y] = [];
    for (let x = 0; x < width; x++) {
      const cost = moveCostOverride[y][x];
      let legacyType: 'empty' | 'difficult' | 'impassable';
      if (cost != null && !Number.isFinite(cost)) {
        legacyType = 'impassable';
      } else if (cost != null && cost > 1) {
        legacyType = 'difficult';
      } else if (mechanics[y][x].movement === 'blocked') {
        legacyType = 'impassable';
      } else if (mechanics[y][x].movement === 'difficult' || mechanics[y][x].movement === 'swim') {
        legacyType = 'difficult';
      } else {
        legacyType = 'empty';
      }
  ...
  // ── Convert PlacedOverlay[] → AtlasOverlay[] (drop z, keep rendering data) ──
  const gameOverlays: AtlasOverlay[] = atlasOverlays.map(ov => ({
    x: ov.x,
    y: ov.y,
    atlas: ov.atlas,
    col: ov.col,
  ...
  // ── Generate starting zones ──
  const zones = generateStartingZones(mechanics, width, height, playerCount, enemyCount);

  // ── Build features from room data ──
  const features: MapFeature[] = generated.rooms.map(room => ({
    name: room.type ? `${room.type} room` : 'room',
    type: 'wall' as const,
    positions: [{ x: room.x, y: room.y }],
    description: `${room.w}×${room.h} room`,
  }));

  // ── Map atlas theme → shared MapTheme ──
  // The atlas gen supports: wilderness, dungeon, indoor, cave, urban
  // The shared type includes more themes but atlas maps only cover the 5 above
  const sharedTheme = (generated.theme === 'cave' ? 'mine' : generated.theme) as
    import('./types').MapTheme;

  const template: EncounterMapTemplate = {
    id: `atlas-${generated.theme}-${generated.seed}`,
    name: `Atlas ${generated.theme.charAt(0).toUpperCase() + generated.theme.slice(1)} Map`,
    theme: sharedTheme,
    subTheme: generated.theme,
    description: `Procedurally generated ${generated.theme} map using atlas tiles (${generated.genTimeMs}ms)`,
    width,
    height,
    terrain,
    startingZones: zones,
